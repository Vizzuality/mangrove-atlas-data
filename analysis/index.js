...
exports.analyse = (req, res) => {
  ...
  // set default or error
  if (fid === undefined) { res.json({error: 'fid is required, try fid=1_2_22'}); }
  // print to console
  ...
  if (analysis_types === undefined) { 
    analysis_types = '["land-cover","mangrove-properties", "mangrove-carbon"]'; }
  analysis_types = JSON.parse('{ "at":' + analysis_types +  ' }').at;
    // print to console
  console.log('info: analysis_types=' + analysis_types);
  console.log(typeof analysis_types);
  ...
  timestamps = JSON.parse('{ "ts":' + timestamps + ' }').ts
  ...
  buffer_distance = parseInt(buffer_distance_m)
  console.log('info: buffer_distance_m=' + buffer_distance_m);
  console.log(typeof buffer_distance_m);
  ...
  // default: false
  // parse request set default or error
  var bestEffort = false
  bestEffort = (req.query.bestEffort === 'true');
  console.log('info: bestEffort=' + bestEffort);
  console.log(typeof bestEffort);
